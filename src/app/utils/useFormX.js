...
export function useFormx(defaultValues, validateSchema, submitCallback) {
  ...
  // const useInput = (name) => useFormInput(name, defaultValues, validateSchema, formValues);
  // const useInput = (name) => useMemo(useFormInput(name, defaultValues, validateSchema, formValues, handleError), [name, formValues.current[name]]);
...
function useFormInput(
  ...
  const handleBlur = useCallback((event) => {
    if (error.hasError !== errors[name].hasError) {
  ...
  }, [error, errors, handleError, name, validateSchema, values]);

  const handleFocus = useCallback(() => {
    !isTouched.current[name] && (isTouched.current[name] = true);
  }, [isTouched, name]);

  // showError = !isVaild && isTouched;
  // const returnHandle = validateSchema.getFieldType(name).validateOnBlur ? { onBlur: handleBlur } : { onChange:handleChange }
  return {
    value,
